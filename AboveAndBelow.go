...
func MouseInteract(g *Game) {
	...
	//Clicking detection
	if mouse_one {
		g.FirstBuffer[world_y][world_x] = g.Index
	}
	if mouse_two {
		g.FirstBuffer[world_y][world_x] = 0
	}
}
...
func (g *Game) DensityCompare(row, col int) bool {
	if row > 0 && g.FirstBuffer[row][col] != 0 && g.FirstBuffer[row-1][col] != 0 && ElementMap[g.FirstBuffer[row][col]].Density > ElementMap[g.FirstBuffer[row-1][col]].Density {
		return true
	}
	return false
}
